...
a.result.views===h?b('[data-counter="su"]').text("0"):b('[data-counter="su"]').text(d(a.result.views))});b.getJSON("http://query.yahooapis.com/v1/public/yql?q="+encodeURIComponent('select * from html where url="https://widgets.getpocket.com/v1/button?count=horizontal&url='+location.href+'" and xpath="*"')+"&format=json&callback=?",function(a){b('[data-counter="po"]').text(d(a.query.results.html.body.div.a.span.em.content))});b.getJSON("https://api.bufferapp.com/1/links/shares.json?url="+encodeURIComponent(location.href)+
"&callback=?",function(a){b('[data-counter="bf"]').text(d(a.shares))});b(c).on("click",".goodshare",function(a){a.preventDefault();goodshare.init(this)})})})(jQuery,window,document);
